import type { ArtistUiType, GenderType } from './types';
...
const MOCK_MALE_ARTISTS: ArtistUiType[] = [
...
const MOCK_FEMALE_ARTISTS: ArtistUiType[] = [
...
export const MOCK_ALL_ARTISTS: ArtistUiType[] = [
  ...MOCK_MALE_ARTISTS,
  ...MOCK_FEMALE_ARTISTS,
];
...
export const fetchRecommendedArtists = async (
  ...
  const pool = gender === 'male' ? MOCK_MALE_ARTISTS : MOCK_FEMALE_ARTISTS;
  return shuffle(pool).slice(0, PICK_COUNT);
};

export const fetchArtistsByIds = async (ids: number[]): Promise<ArtistUiType[]> => {
  return MOCK_ALL_ARTISTS.filter((artist) => ids.includes(artist.id));
};
