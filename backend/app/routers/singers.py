...
async def get_random_singers(
    gender: str = Query(..., description="성별 필터 (male | female)"),
    limit: int = Query(6, ge=1, le=50, description="반환할 가수 수"),
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    """성별 필터 후 차단 제외 랜덤 가수 목록 반환"""
    service = SingerService(db)
    singers = await service.get_random_singers(
